...
import { frozenProtocol, forgedFrozenProtocol } from "./fixtures.mjs";
...
test("IMP-19 §25.2.3 · un protocolo con hash válido pero contrato ausente no está congelado-autorizado", () => {
  // La integridad content-addressed sólo prueba que el artefacto no cambió; el
  // gate debe re-validar el contrato semántico (§12.3) para autorizarlo.
  const noMixture = forgedFrozenProtocol({ mixtureDeclaration: null });
  const mixtureOutcome = assertProtocolFrozenBeforeEvaluation({ protocol: noMixture, evaluatedAtUtc: "2026-02-01T00:00:00Z" });
  assert.equal(mixtureOutcome.ok, false);
  assert.equal(mixtureOutcome.code, "PROTOCOL_CONTRACT_INVALID");
  assert.ok(mixtureOutcome.errors.some((error) => error.code === "MISSING_MIXTURE_DECLARATION"));

  const noCadence = forgedFrozenProtocol({ cadence: null });
  const cadenceOutcome = assertProtocolFrozenBeforeEvaluation({ protocol: noCadence, evaluatedAtUtc: "2026-02-01T00:00:00Z" });
  assert.equal(cadenceOutcome.ok, false);
  assert.equal(cadenceOutcome.code, "PROTOCOL_CONTRACT_INVALID");
  assert.ok(cadenceOutcome.errors.some((error) => error.code === "MISSING_CADENCE"));
});

test("IMP-19 §25.2.3 · una mezcla con fuente no canónica o cadence incompleta no autoriza", () => {
  const badSource = forgedFrozenProtocol({ mixtureDeclaration: { declarationVersion: "mix-v1", declaredBy: "test-fixture", weights: { NOT_A_SOURCE: 1 } } });
  const sourceOutcome = assertProtocolFrozenBeforeEvaluation({ protocol: badSource, evaluatedAtUtc: "2026-02-01T00:00:00Z" });
  assert.equal(sourceOutcome.ok, false);
  assert.equal(sourceOutcome.code, "PROTOCOL_CONTRACT_INVALID");
  assert.ok(sourceOutcome.errors.some((error) => error.code === "UNKNOWN_SOURCE_IN_WEIGHTS"));

  const missingTrigger = forgedFrozenProtocol({ cadence: { reviewTrigger: "campaign_close" } });
  const triggerOutcome = assertProtocolFrozenBeforeEvaluation({ protocol: missingTrigger, evaluatedAtUtc: "2026-02-01T00:00:00Z" });
  assert.equal(triggerOutcome.ok, false);
  assert.equal(triggerOutcome.code, "PROTOCOL_CONTRACT_INVALID");
  assert.ok(triggerOutcome.errors.some((error) => error.code === "MISSING_TRAINING_TRIGGER"));
});