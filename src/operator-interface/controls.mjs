...
// autoridad concreta que la otorga y un instante. Sin ella no hay comando.
function validateAuthorization(authorization, errors) {
  ...
  return deepFreeze({ authorityRef: authorization.authorityRef, grantedBy: authorization.grantedBy, grantedAtUtc: grantedAt.utc });
}

// Los comandos de governance exigen el receipt aplicable (§26.5).
function validateGovernanceReceipt(receipt, errors) {
  ...
  return deepFreeze({ receiptRef: receipt.receiptRef, receiptSha256: receipt.receiptSha256.toLowerCase() });
}
...
export function authorizeOperatorCommand(input) {
  ...
  const errors = [];
  const authorization = validateAuthorization(input.authorization, errors);
  ...
  let receipt = null;
  if (isGovernance) {
    receipt = validateGovernanceReceipt(input.receipt, errors);
  }
  // §26.2/§26.3: una intervención se vincula a la recomendación original y a la
  // acción efectiva; sin ellas no es una intervención trazable.
