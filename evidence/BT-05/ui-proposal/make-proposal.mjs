...
const STATES = {
  idle: { running: false, current: null, latest: { runId: ILLUSTRATIVE_RUN_ID, status: "SUCCEEDED", finishedAt: "2026-09-25T15:00:04.000Z", failure: null, retention: { state: "CURRENT", supersededBy: null } } },
  running: { running: true, current: { runId: ILLUSTRATIVE_RUN_ID, status: "RUNNING", finishedAt: null, failure: null, retention: { state: "NONE", supersededBy: null } }, latest: null },
};
...
try {
  ...
  for (const [name, status] of Object.entries(STATES)) {
    const html = withBacktestJobControl(page, status).replace(/<body([^>]*)>/, `<body$1>${BANNER}`);
