...
| Archivo | `PROCUREMENT_RESEARCH_CANONICAL_ENGINEERING_SPEC_v1_1.md` |
| Versión | 1.1 — targeted patch D1–D5 sobre la baseline v1.0 |
| Fecha | 2026-09-18 |
| Status | Baseline v1.0 preservada y patch D1–D5 FROZEN incorporado. P1–P7 no se reabren. Contratos de extensión, integración externa, interfaz y handoff cerrados; implementación, audit y evidence pendientes según §§20–26. No constituye validación empírica ni autorización de producción. |
| Mandante | Bru |
...
| Informe de auditoría | `PROCUREMENT_RESEARCH_CANONICAL_CONSOLIDATION_REPORT_v1_1.md`; registra el estado consolidado. `PROCUREMENT_RESEARCH_v1_0_to_v1_1_PATCH_REPORT.md` audita el cambio puntual. Ninguno define una arquitectura alternativa. |
| Evidencia inspeccionada | 19 fuentes completas acumuladas: D00–D17 leídas en la consolidación v1.0 (15 PDF, 185 páginas, y 3 textos) más D18, leído íntegramente para este patch. Los dos entregables v1.0 son baselines derivadas y no se cuentan otra vez como fuentes independientes. No se inspeccionaron datos de mercado, BruNode, Paperclip/JEV, repositorios externos ni resultados de ejecución. |
...
Cambiar identidad de Strategy, semántica BUY/WAIT, población, baseline, benchmark, reward, métricas, criterios, protocolo OOS o envelope requiere cambio explícito de versión y trazabilidad `Supersedes / Canonical`. Completar un campo con su valor auditado no permite cambiar su significado. Todos los receipts anteriores se conservan. Durante implementación, una contradicción real o necesidad de alterar semántica frozen activa `SPEC_CHANGE_REQUEST` (§20.2.12); Astra y workers no aprueban ese cambio por sí mismos. D18 amplía el contrato documental de handoff, sin certificar la instalación actual de Paperclip.
...
| Astra / Paperclip handoff | Extender la oficina existente y ejecutar el canonical IMP graph mediante WORK-PACKETs, reviews y receipts (§20.2/§25.2); routing es asunto de oficina. |
...
El **volumen total de la obligación ya es conocido** según P1.1. No constituye una OPEN DECISION. Su valor tipado debe provenir del mandato real y quedar materializado en el registro de campaña. El corpus anterior contiene cantidades comunicadas con unidades incompletas; esas anotaciones históricas no autorizan conversiones ni sustituyen un contrato ejecutable. Esta especificación no asigna cantidades numéricas no reconciliadas.
...
| Obligación | Volumen total conocido, unidad, periodo y perfil de entrega; enmiendas/cancelaciones documentadas cuando existan | AUDIT-DEPENDENT |
...
El análisis de solapamiento incluye Paperclip, S1–S5, Candidate Policy y cualquier capacidad existente relevante. Duplicar una función de Paperclip exige análisis explícito de valor; la disponibilidad de un componente externo no justifica rediseñar la oficina ni duplicar su autoridad.
...
| **Overlap assessment** | Solapamiento con Paperclip, S1–S5, Candidate Policy u otra capacidad existente. |
...
## 20.2 Astra / Paperclip Implementation Handoff Contract

**Estado:** CANONICAL / FROZEN — D18 D4, approved with required dependency-semantics correction. El contrato define cómo ejecutar el backlog canónico; no acredita que su binding con el office esté implementado ni auditado. Paperclip conserva su arquitectura existente. La inspección de sus interfaces y la materialización del handoff son trabajo de implementación/auditoría, no decisiones conceptuales nuevas.
...
### 20.2.1 Existing Paperclip roles
...
| **Paperclip** | Persistent queue, continuation, dispatch, state, recovery y observability layer |
...
El replenisher de Paperclip queda **SPEC-bound** para Procurement. Cuando READY cae por debajo del threshold ya configurado, despierta Command para consultar qué IMPs canónicos son elegibles, no para inventar una nueva agenda.
...
La SPEC define **WHAT and WHY**. Astra/Paperclip gobierna **HOW** el trabajo de implementación aprobado se descompone, delega, revisa, registra y continúa. Los subtasks, packets y execution state ejecutan §25; no sustituyen sus objetivos ni constituyen otro plan arquitectónico.

El binding debe contrastarse con el office actual antes de declarar cumplimiento: roles/routing, Project ON/OFF, queue/state/recovery, formatos de trabajo/review/receipts y continuation existentes. Documentar este contrato no demuestra que esa inspección haya ocurrido. La implementación extiende lo existente sólo donde el mapping detecte una carencia; no reconstruye Paperclip por preferencia ni presume capacidades verificadas.
...
No hay evidencia adjunta de un run de mercado, auditoría real completada, suite P6 aprobada, Shadow o activación productiva. «No demostrado en el corpus» no significa que un sistema externo no exista: significa que esta SPEC no puede certificarlo. Las descripciones de cada celda evitan asignar un resultado DATA_READY, PASS o autonomía a material no auditado.
...
| Procurement Contract | CANONICAL / FROZEN — P1 | AUDIT-DEPENDENT | Contrato conceptual materializable; instancia real pendiente | Cobertura real no demostrada | DEP-01–04 | IMP-02 |
...
| Quarterly / Monthly scoring | CANONICAL / FROZEN | Requiere B/H/campañas válidos | IMPLEMENTATION-READY para fórmulas y fixtures | EVIDENCE-DEPENDENT — aplicar gates | DEP-08/12–14 | IMP-08/16 |
| Point-in-Time Contract | CANONICAL / FROZEN | Audit real pendiente | IMPLEMENTATION-READY para separación temporal y versionado | Integridad por verificar | DEP-06/07 | IMP-03/06 |
| Data Sufficiency Matrix | CANONICAL / FROZEN | Celdas y readiness por candidato AUDIT-DEPENDENT | IMPLEMENTATION-READY para estructura | Suficiencia no demostrada | DEP-06/07/10 | IMP-03 |
...
| Astra / Paperclip Implementation Handoff | CANONICAL / FROZEN — D4 | Oficina actual y estado real de IMPs AUDIT-DEPENDENT | Contrato implementable extendiendo Paperclip; integración no acreditada | Pruebas de elegibilidad, receipts/review y continuidad pendientes | DEP-27 para binding real; no exige código nuevo antes del audit | IMP-25/26; §20.2 |
...
| CCR-08 | INSUFFICIENT_EVIDENCE | Números heredados 10 MW Monthly Power/Gas, «10 Energy» Power Quarterly, 60 MW Gas Quarterly, D08 p.4/D15 p.2. | P1 cierra total conocido, pero el corpus no contiene todas las fichas operativas. Conservar cifras como dato comunicado, sin convertir «10 Energy» a MW ni MW a MWh; DEP-01/02. |
...
| CCR-13 | INSUFFICIENT_EVIDENCE | 2020–2026 comunicado y 2018 mencionado como ejemplo, D15 p.3. | D12 p.2 corrige alcance comunicado: 2018 no disponible; 2020–2026 sigue sujeto a audit por contrato/fuente/resolución. No son siete años completos probados. |
...
|---|---|---|---|
| DEP-01 — AUDIT-DEPENDENT | Instancias de mandato: producto/hub/contrato, total conocido materializado, unidad, delivery profile/horas, liquidación física/financiera. | Cantidades ejecutables, comparabilidad B/H y EUR; no inferir producto por residencia del cliente. | Responsable de procurement + contratos reales; ficha reconciliada por obligación. D04 P1/P4.1; D08 p.4. |
...
| DEP-06 — AUDIT-DEPENDENT | Inventario real de precios y contratos, cobertura 2020–2026, resolución, unidades, missingness, revisiones y permisos. | Data-readiness y campañas elegibles; no suponer historia por declaración general. | Data owner/custodio; Data Sufficiency Matrix con requisitos críticos. D04 P4.1/P4.4. |
| DEP-07 — AUDIT-DEPENDENT | Publication y policy-consumable time demostrables; historia de forecasts, expected/surprise, fundamentales y eventos. | Replay causal; campos sin prueba histórica quedan unavailable/proxy válido/forward-only. | Fuente contemporánea verificable y manifests versionados. D04 P4.2–P4.4; D13 §§3,7. |
| DEP-08 — AUDIT-DEPENDENT | Reproducir/reconciliar Benchmark Foundation por campaña y fecha con feed oficial o autorizado; cobertura y sustitución proxy. | B final y admisión económica; reproducibilidad proxy no prueba equivalencia. | Fuente/reconciliación benchmark, auditoría independiente del engine. D04 P2.1; D16 §§3–5; D02 P6.6. |
...
| DEP-10 — AUDIT-DEPENDENT | Capacidades/interfaces del motor existente y rights/IP del entorno y fuentes. | Elegir menor herramienta suficiente y permitida; no construir plataforma por preferencia. | Capability audit y comprobación de uso autorizado, ejemplo sintético reconciliable. D04 P4.5; D08 pp.3,11. |
...
El canonical IMP graph es el único plan de implementación. Se conservan IMP-01–IMP-24 y se añaden cinco unidades acotadas, IMP-25–IMP-29, para D1–D4; sus números son identidades, no una obligación de terminar todos los IDs anteriores antes de iniciar un trabajo independiente. Astra selecciona exclusivamente trabajo elegible conforme a §20.2.4 y §25.2.
...
| IMP-25 | Auditar Paperclip existente y mapear su workflow al handoff canónico, preservando la oficina. | SPEC vigente y material/configuración/estado reales accesibles bajo autoridad existente | Capability/compatibility mapping factual, fuentes verificadas, brechas y límites del audit | Project ON/OFF, routing, Queue/READY, dispatch/recovery, review y receipts quedan contrastados; desconocidos visibles. Audit aceptado no afirma capacidad inexistente ni cierra todo DEP-27 por rellenar campos. | Arquitectura de oficina/routing; P1–P7; no rediseñar Paperclip, inventar configuración o exigir binding implementado para auditarlo. | §§20.2,24; D18 D4 |
...
Esta tabla es la única especificación de dependencias por IMP. Conserva los 24 IDs y objetivos existentes y añade IMP-25–IMP-29 para los contratos D1–D4; sus outputs, acceptance tests y MUST NOT CHANGE están en §25.1. Sustituye la interpretación de una mención genérica «DEP» como prerequisite. La selección de trabajo y la aceptación siguen el Astra / Paperclip Implementation Handoff Contract de §20.2.
